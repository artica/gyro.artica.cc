...
// Generates a random 32 bit ID with a minimum value
function createId(min_id)
{
    var min = min_id ? min_id : 0;
	return randomInt(min, 4294967296);
}
...
function expandDiv(div, time)
{
    ...
    $('#'+div).stop().
            ...
            animate( {height:height}, time, function() 
            {
                $('#'+div).css({height:'auto'});         
                // Adjust the properties window width depending on the scrollbar
                _ui.adjustPropertiesWidth();
            });
}
...
function collapseDiv(div, time)
{
    ...
    $('#'+div).stop().
            ...
            { 
                $('#'+div).css({height:height, display:'none'}); 
                // Adjust the properties window width depending on the scrollbar
                _ui.adjustPropertiesWidth();
            } );
}
...
function createHtmlSelectorTranslated(id, class_id, options, current, number)
{
    var html = "<select ";

    if (id) html +="id='"+id+"' ";
    if (class_id) html += "class='"+class_id+"' ";
    if (number) html += "number='"+number+"' ";
    html += ">";
...
function toggleSectionVisibility(event, data)
{
    //console.log(event);
    //console.log(data);
    ...
    if ($('#'+name).is(":visible"))
    {
        ...
        if (data) data.hidden_elements[name] = true;
    }
    ...
    else
    {
        ...
        // If we have a data element associated, save the visibility data
        if (data) data.hidden_elements[name] = false;
    }

    console.log(data.hidden_elements);
}
...
function createHtmlSelectorsDataValue(id, source, value)
{
    ...
    {
        case DATA_SOURCE_TYPE_CONSTANT:
            html += "<input type='number' id='"+id+"_value' class='selector_data_value' min='-2147483648' max='2147483647' value='"+(isNaN(value)? 1: value)+"'></input>";
        break;
...
function drawSquareImage( ctx, image, center_x, center_y, size ) 
{
    ...
    var offset = size/2;
    ctx.drawImage(image, center_x-offset , center_y-offset, size, size);
}

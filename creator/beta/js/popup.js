...
/*
var ORIENTATION_UP = 1;
var ORIENTATION_DOWN = 2;
var ORIENTATION_LEFT = 3;
var ORIENTATION_RIGHT = 4;

function openTooltip(message, x, y, orientation)
{		
	//console.log('---------------------------openTooltip:'+message);

	$('#tooltip_message_content').html(message);
	...
	$('#tooltip_message_div').stop().css({display:'block',  opacity:0.0, left:left, top:top, width:'auto', height:'auto'});
	

	$('#tooltip_message_div').animate({opacity: 1.0}, ANIMATION_TIME_POPUP_CHANGE,
		function()
		{		
			// Callback to close the warning tooltip when it's clicked
			//$(document).on('mousemove', closeTooltip);
		});
...
function closeTooltip()
{
	...
	$(document).off('mousemove', closeTooltip);
	$('#tooltip_message_div').animate({opacity:0.0}, ANIMATION_TIME_POPUP_CHANGE,
		function()
			{
				$('#tooltip_message_div').css({display:'none'});
				//´Remove the callback to close the warning tooltip when it's clicked
			});
	;
}
